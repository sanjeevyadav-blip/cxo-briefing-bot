...
Shows a headline card per story -- the headline and source in the upper
half, the spoken line as a caption below -- and changes the card as the
conversation moves on, so it reads like a deck rather than subtitles.
...
import story_match
from ass_slides import write_ass
...
LISTS = {
    "business": "shortlist.json",
    "world": "world_shortlist.json",
    "explainer": "shortlist.json",
}


def load_stories(kind):
    path = DATA / LISTS[kind]
    if not path.exists():
        return []
    items = json.loads(path.read_text())
    return items[:2] if kind == "explainer" else items
