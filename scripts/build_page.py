...
def main():
    ...
    video_ok = MP4.exists()
    if video_ok:
        shutil.copy(MP4, EPISODES / f"{today}.mp4")
    ...
    if NOTES.exists():
        ...
        if video_ok:
            header += f"Video: [episodes/{today}.mp4](../episodes/{today}.mp4)\n\n"
        (NOTES_ARCHIVE / f"{today}.md").write_text(header + NOTES.read_text())

    episode_files = sorted(EPISODES.glob("*.mp4"), reverse=True)
    ...
    if video_ok:
        player = (f'<video controls preload="metadata" src="episodes/{today}.mp4" '
                  f'style="width:100%;border-radius:6px"></video>')
    else:
        player = "<p>(No episode today -- check the Actions log.)</p>"
    ...
    html = (
        ...
        f'<h1>{TITLE}</h1><p class="date">{today}</p>{player}'
    ...
    print(f"Built page for {today}, video_ok={video_ok}, "
