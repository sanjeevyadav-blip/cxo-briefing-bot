...
def ask(client, system_prompt, temperature, max_tokens):
    resp = client.chat.completions.create(
        ...
        # gpt-oss models are reasoning models: reasoning tokens are drawn
        # from the same max_tokens budget and default to "medium" effort,
        # which starved the actual answer. Keep it minimal -- this task is
        # writing, not problem-solving.
...
def main():
    ...
    raw = ask(client, SYSTEM_PROMPT, 0.8, 5000)
    # Written before parsing so a bad response is always inspectable in the
    # uploaded artifact, not just described in an error message.
    (ROOT / "data" / "script_raw.txt").write_text(raw)
    turns = parse_script(raw)
    if not turns:
        raise SystemExit(
            "Output didn't parse into [A]/[B] turns. "
            f"Got {len(raw)} chars. First 400:\n{raw[:400]}"
        )
    ...
    try:
        # Groq's free tier caps tokens-per-minute (8000 at time of writing).
        # The two calls together exceed that if fired back-to-back, so wait
        # out the window before the second one.
        time.sleep(65)
        raw_sum = ask(client, SUMMARY_SYSTEM_PROMPT, 0.4, 2500)
